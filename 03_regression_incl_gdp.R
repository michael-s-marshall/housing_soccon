...
con_con2 <- lmer(soc_con ~ white_british + 
                  religion + male + higher_ed +
                  own_outright + social_housing +
                  private_renting + tenure_other + Age + 
...
con_con3 <- lmer(soc_con ~ white_british + 
                   religion + male + higher_ed +
                   own_outright + social_housing +
                   private_renting + tenure_other + Age + 
                   affordability + gdp_growth_pct_5 +
                   (1|la_code),
                 data = df_con, REML = FALSE)
summary(con_con3)
...
econ_con <- lmer(econ_right ~ white_british +
                   ...
                   tenure_other +
                   Age + affordability + gdp_capita + (1|la_code),
                 data = df_econ, REML = FALSE)
summary(econ_con)

anova(econ_multi, econ_con)

econ_con2 <- lmer(econ_right ~ white_british +
                   religion + male + higher_ed +
                   own_outright + social_housing + private_renting +
                   tenure_other +
                   Age + affordability + gdp_growth_pct + (1|la_code),
                 data = df_econ, REML = FALSE)
summary(econ_con2)

anova(econ_multi, econ_con2)

econ_con3 <- lmer(econ_right ~ white_british +
                    religion + male + higher_ed +
                    own_outright + social_housing + private_renting +
                    tenure_other +
                    Age + affordability + gdp_growth_pct_5 + (1|la_code),
                  data = df_econ, REML = FALSE)
summary(econ_con3)
