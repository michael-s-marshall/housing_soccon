...
afford <- afford %>% 
  ...
  mutate(affordability = log(`2022`),
         afford_lag_one = log(`2021`),
         afford_lag_two = log(`2020`)) %>% 
...
df_equal %>% 
  filter(is.na(foreign_per_1000)) %>% 
  select(la_code) %>% 
  unique() %>% 
  as_vector() # missing = City of London and Isles of Scilly
